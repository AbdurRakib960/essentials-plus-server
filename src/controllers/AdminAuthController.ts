...
class AdminAuthController {
  ...
  // @POST="/admin/signup" // ✔️ checked
  signupAdmin: RequestHandler = async (req, res) => {
    const value = await this.validators.signup.parseAsync(req.body);

    const admin = await prisma.admin.findUnique({ where: { email: value.email } });

    if (admin) throw new Error("Admin already exist");
  ...
  // @POST="/admin/signup/verify" // ✔️ checked
  verifyEmail: RequestHandler = async (req, res) => {
    ...
    if (!admin) throw new Error("Invalid link");

    await prisma.admin.update({ where: { id: data.id }, data: { verified: true } });
